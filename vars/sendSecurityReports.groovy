...
def call(Map config = [:]) {
    ...
    withCredentials([
        ...
        if [ -f "${reportDir}/nuclei-report.html" ]; then
            ...
            HTML_ATTACHMENTS_LIST="\${HTML_ATTACHMENTS_LIST}<li><b>Nuclei DAST HTML Report:</b> Attached (nuclei-report.html)</li>"
        fi

        if [ -f "${reportDir}/nuclei-report.txt" ]; then
            ATTACHMENTS="\${ATTACHMENTS} ${reportDir}/nuclei-report.txt"
            HTML_ATTACHMENTS_LIST="\${HTML_ATTACHMENTS_LIST}<li><b>Nuclei DAST Text Report:</b> Attached (nuclei-report.txt)</li>"
        fi
