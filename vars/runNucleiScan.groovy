...
def call(Map config = [:]) {
    ...
    # 6. Consolidate Nuclei native markdown reports into comprehensive report files
    if [ -d "${reportDir}/nuclei-md" ] && [ "\$(ls -A "${reportDir}/nuclei-md" 2>/dev/null)" ]; then
        ...
        } > "${reportDir}/nuclei-report.md"

        # Also copy as txt so both .md and .txt formats exist
        cp "${reportDir}/nuclei-report.md" "${reportDir}/nuclei-report.txt"
    fi
    ...
    if [ ! -s "${reportDir}/nuclei-report.md" ]; then
        {
            ...
            echo "---"
            echo ""
            echo "### Status: Completed - No vulnerabilities identified matching configured severities."
        } > "${reportDir}/nuclei-report.md"

        cp "${reportDir}/nuclei-report.md" "${reportDir}/nuclei-report.txt"
    fi

    echo "Nuclei Markdown report generated at ${reportDir}/nuclei-report.md"
